...
travelsRouter.get("/:travelId", JWTAuthMiddleware, async (req, res, next) => {
  try {
    const trip = await NewTripModel.findById(req.params.travelId).populate({
      path: "userId",
    });
    if (trip) {
      res.send(trip);
      ...
      next(
        createHttpError(404, `trip with id ${req.params.travelId} not found!`)
      );
    }
  } catch (error) {
    next(error);
...
travelsRouter.get(
  "/:travelId/pakinglist/:pakinglistId",
  JWTAuthMiddleware,
  async (req, res, next) => {
    try {
      const trip = await NewTripModel.findById(req.params.travelId);
      if (trip) {
        const singlePakinglist = trip.pakingLists.find(
          (single) => single._id.toString() === req.params.pakinglistId
        );
        if (singlePakinglist) {
          ...
          next(
            createHttpError(
              404,
              `pakinglist with id ${req.params.pakinglistId} not found!`
            )
          );
        }
      } else {
...
travelsRouter.put(
  ...
  async (req, res, next) => {
    try {
      const trip = await NewTripModel.findById(req.params.travelId);
      if (trip) {
        const index = trip.pakingLists.findIndex(
          (singlePar) => singlePar._id.toString() === req.params.pakinglistId
        );

        if (index !== -1) {
          trip.pakingLists[index] = {
            ...trip.pakingLists[index].toObject(),
            ...req.body,
          };
          await trip.save();
          res.send(trip);
...
travelsRouter.get(
  "/:travelId/accommodation/:accommodationId",
  JWTAuthMiddleware,
  async (req, res, next) => {
    try {
      const trip = await NewTripModel.findById(req.params.travelId);
      if (trip) {
        const singleAccommodation = trip.accommodations.find(
          (single) => single._id.toString() === req.params.accommodationId
        );
...
travelsRouter.put(
  ...
  async (req, res, next) => {
    try {
      const trip = await NewTripModel.findById(req.params.travelId);
      if (trip) {
        const index = trip.accommodations.findIndex(
          (singlePar) => singlePar._id.toString() === req.params.accommodationId
        );

        if (index !== -1) {
          trip.accommodations[index] = {
            ...trip.accommodations[index].toObject(),
            ...req.body,
          };
          await trip.save();
          res.send(trip);
